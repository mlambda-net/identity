...
func (s server) Start() {
  ...
  go func() {
    ...
    s.remote.Register("user", user.NewUserProps(s.config), true, []string{"identity_admin"})
    s.remote.Register("auth", auth.NewAuthProps(s.config), false, []string{})
    s.remote.Register("app", app.NewAppActor(s.config), true, []string{"identity_admin"})
    s.remote.Register("role", roles.NewRolesProps(s.config), true,[]string{"identity_admin"})
